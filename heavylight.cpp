...
// 1 based
//  ** If values on edges, make sure to assign ids to edges **
...
struct HeavyLight {
	...
	int n, root, isValueOnEdge = 0; //1 if value on edge
	...
	void init(bool edge) { // 1 if queries are on edges, 0 if on nodes
		isValueOnEdge = edge;
